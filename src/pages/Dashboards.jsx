...
import { widgetGroupBy } from '@/lib/dashboards'
...
// the first metric column is charted, every column is tabled.
function WidgetCard({ widget }) {
  if (widget.error) {
    return (
      <div className="rounded-2xl border border-amber-100 bg-amber-50/50 p-4 h-full">
        <p className="text-xs font-bold text-slate-700 mb-1">{widget.title || 'Widget'}</p>
              ...
              <tbody>
                {result.rows.map((r, i) => (
                  <tr key={i} className="border-b border-slate-50 last:border-0">
                    <td className="px-2.5 py-1.5 font-semibold text-slate-700 whitespace-nowrap">{dimCell(r)}</td>
                    {metricCols.map((m) => (
...
function DashboardDetail({ id, onBack, onChanged }) {
  ...
  const load = useCallback(() => {
    ...
    api.runDashboard(id)
      .then((r) => { setRun(r); setStatus('done') })
      ...
      {status === 'done' && run && (
        (run.widgets || []).length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {run.widgets.map((w) => <WidgetCard key={w.id} widget={w} />)}
          </div>
        )
      )}
    </div>
