...
// own-numbers / client-framed on the dashboard.
//
// Sample numbers below are shaped EXACTLY like pulseDiagnose()'s output
// (drivers:[{metric,pct,share,share_pct}], lead, direction) so this is a faithful
// look at the real surface. Reachable at /pulse-preview (no auth) for review;
// the shared DriverBreakdown/DriverShareRow lift straight into ClientView +
// Intelligence in steps 2c/2d. Client names here are fictional on purpose.
// ============================================================
import { ArrowUp, ArrowDown, Minus, Activity, Sparkles, Clock } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 p-6 sm:p-10">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <p className="text-[10px] font-black uppercase tracking-widest text-brand-600">Design preview · intel-v7 (2)</p>
          <h1 className="text-2xl font-black text-slate-900 mt-1">Daily Pulse → driver diagnosis</h1>
          <p className="text-sm text-slate-500 font-medium mt-1.5 max-w-3xl leading-relaxed">
            The pulse already says <span className="font-bold text-slate-700">what</span> moved this week. This adds the{' '}
            <span className="font-bold text-slate-700">why</span> — decomposing each composite move (revenue ≡ spend × ROAS,
            jobs ≡ leads × close rate) into its exact drivers in log space. No model, no tuning; every figure traces to a stored
            daily fact. Sample numbers; client names fictional.
          </p>
        </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <div className="px-4 py-2.5 bg-sky-50/30 border-t border-slate-50">
                <p className="text-[11px] font-medium text-slate-400 leading-relaxed">
                  When a composite metric moves, the pulse now names the{' '}
                  <span className="font-bold text-sky-600">lever behind it</span> — leads vs close rate, spend vs ROAS — days
                  before the Monday recap. Agency-only; a client sees only its own.
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <p className="text-[10px] text-slate-400 mt-3.5 pt-3 border-t border-slate-50 leading-relaxed">
                ...
                so you see the cause days before the week officially closes.
              </p>
            </div>
          </div>
        </div>
