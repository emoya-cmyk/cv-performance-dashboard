...
//
// Ported verbatim (behaviour-identical) from agency-performance-dashboard's
// api/lib/authSecurity.js, with the only change being that the constants the
// originals hard-coded (the dev-secret fallback literal, the password floor) are
// now parameterizable — but the defaults are exactly agency's, so a caller that
// passes nothing behaves byte-for-byte like agency today.
//
...
//
// opts.devSecretFallback — the literal that counts as "still the dev fallback"
// (defaults to DEV_SECRET_FALLBACK so behaviour matches agency exactly). This is
// the one knob a downstream repo with a different fallback literal would set.
function checkProductionSecret(env = {}, opts = {}) {
  if (env.NODE_ENV !== 'production') return { ok: true, error: null }
  const fallback = opts.devSecretFallback != null ? opts.devSecretFallback : DEV_SECRET_FALLBACK
...
// Boot guard convenience: assert a usable JWT secret or THROW. agency calls
// checkProductionSecret at bootstrap and exits on !ok; this wraps that into a
// single throwing call for callers that prefer fail-fast at startup. Behaviour:
// in production with a missing/fallback secret it throws (with the guard's
// message); otherwise it returns the effective secret (the configured one, or the
// dev fallback outside production — exactly what agency's middleware/auth.js uses
// when JWT_SECRET is unset).
function assertJwtSecret(env = {}, opts = {}) {
  const fallback = opts.devSecretFallback != null ? opts.devSecretFallback : DEV_SECRET_FALLBACK
  const { ok, error } = checkProductionSecret(env, { devSecretFallback: fallback })
